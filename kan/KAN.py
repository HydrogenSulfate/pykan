...
class KAN(nn.Module):
    ...
    def __init__(
        ...
        for l in range(self.depth):
            ...
            scale_base = scale_base_mu * 1 / np.sqrt(width[l]) + scale_base_sigma * (
                torch.randn(
                    width[l],
                    width[l + 1],
                )
    ...
    def train(
        ...
        def closure():
            ...
            if sglr_avoid == True:
                ...
                train_loss = loss_fn(
                    pred[id_], dataset["train_label"][train_id][id_].to(self.device)
                )
            else:
                train_loss = loss_fn(
        ...
        for _ in pbar:
            ...
            if _ % grid_update_freq == 0 and _ < stop_grid_update_step and update_grid:
                self.update_grid_from_samples(
                    dataset["train_input"][train_id].to(self.device)
                )
            ...
            test_loss = loss_fn_eval(
                self.forward(dataset["test_input"].to(self.device)),
                dataset["test_label"].to(self.device),
            )
