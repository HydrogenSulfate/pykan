...
class KANLayer(nn.Module):
    ...
    def __init__(
        ...
        self.scale_base = torch.nn.Parameter(
            torch.ones(in_dim, out_dim) * scale_base * mask
        ).requires_grad_(
            sb_trainable
        ...
        self.scale_sp = torch.nn.Parameter(
            torch.ones(in_dim, out_dim) * scale_sp * mask
        ).requires_grad_(
            sp_trainable
        )  # make scale trainable
    ...
    def forward(self, x):
        ...
        y = coef2curve(
            x_eval=x, grid=self.grid, coef=self.coef, k=self.k
        )  # y shape: (batch, in_dim, out_dim)

        postspline = y.clone().permute(
            0, 2, 1
        )  # postspline shape: (batch, out_dim, in_dim)
