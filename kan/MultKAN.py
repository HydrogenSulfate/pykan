...
class MultKAN(nn.Module):
    ...
    def __init__(
        ...
        for l in range(self.depth):
            # splines
            scale_base = scale_base_mu * 1 / np.sqrt(width_in[l]) + scale_base_sigma * (
                torch.randn(width_in[l], width_out[l + 1]) * 2 - 1
            ) * 1 / np.sqrt(width_in[l])
            sp_batch = KANLayer(
                in_dim=width_in[l],
        ...
        for l in range(depth):
            ...
            for j in range(out_dim):
                for i in range(in_dim):
                    ...
                    model_load.symbolic_fun[l].funs_avoid_singularity[j][
                        i
            ...
            for i in range(n):
                if isinstance(in_vars[i], sympy.Expr):
                    plt.gcf().get_axes()[0].text(
                        1 / (2 * (n)) + i / (n),
                        -0.1,
                        f"${latex(in_vars[self.input_id[i]])}$",
        ...
        for i in range(len(self.acts_scale) - 1):
            ...
            if mode == "auto":
                ...
                overall_important_up = torch.zeros(
                    self.width_in[i + 1], dtype=torch.bool
                )
            ...
            if self.mult_homo == True:
                ...
                for j in range(overall_important_up[self.width[i + 1][0] :].shape[0]):
                    ...
                    overall_important_down = torch.cat(
                        [overall_important_down, torch.tensor([active_bool] * arity)]
                    )
                    if active_bool:
    ...
    def expand_depth(self):
        ...
        self.node_bias.append(
            ...
            ).requires_grad_(self.affine_trainable)
        )
            ...
            if sum_bool:
                if added_dim == "out":
                    ...
                    self.node_scale[l].data = torch.cat(
                        [torch.ones(n_added_nodes), self.node_scale[l].data]
                    )
                    self.node_bias[l].data = torch.cat(
                        [torch.zeros(n_added_nodes), self.node_bias[l].data]
                    )
                    self.subnode_scale[l].data = torch.cat(
                        [torch.ones(n_added_nodes), self.subnode_scale[l].data]
                    )
                    self.subnode_bias[l].data = torch.cat(
                        [torch.zeros(n_added_nodes), self.subnode_bias[l].data]
                    )
                ...
                if added_dim == "out":
                    ...
                    self.node_scale[l].data = torch.cat(
                        [self.node_scale[l].data, torch.ones(n_added_nodes)]
                    )
                    self.node_bias[l].data = torch.cat(
                        [self.node_bias[l].data, torch.zeros(n_added_nodes)]
                    )
                    self.subnode_scale[l].data = torch.cat(
                        [self.subnode_scale[l].data, torch.ones(n_added_subnodes)]
                    )
                    self.subnode_bias[l].data = torch.cat(
                        [self.subnode_bias[l].data, torch.zeros(n_added_subnodes)]
                    )
